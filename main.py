...
def enter_and_analyze_company(conn):
    ...
    top_low_deps = top_low_per_department(company_data)
    print("\nTop & lowest employees per department:")
    for dep, data in top_low_deps.items():
        print(f"{dep} - Top: {data['top_employee']} | Low: {data['min_employee']}")

    above_deps, below_deps = department_above_avg(company_data)
    print(f"\nDepartments above company avg: {above_deps}")
    print(f"Departments below company avg: {below_deps}")
